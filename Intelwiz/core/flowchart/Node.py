...
class Node(QtCore.QObject):
    ...
    def __init__(self, name, terminals=None, allowAddInput=False, allowAddOutput=False, allowRemove=True,allowMergeNodes=False,allowExpandNodes=False):
        """
        ==============  ============================================================
        Arguments
...
class NodeGraphicsItem(GraphicsObject):
    ...
    def buildMenu(self):
        ...
        if not self.node._allowRemove:
            a.setEnabled(False)
        a = self.menu.addAction("Merge Nodes", self.mergeNodesFromMenu)
        if not self.node._allowMergeNodes:
            a.setEnabled(False)
        a = self.menu.addAction("Expand Node", self.expandeNodeFromMenu)
        if not self.node._allowExpandNodes:
            a.setEnabled(False)
        a = self.menu.addAction("Save Node", self.saveNodesFromMenu)
        a.setEnabled(True)        
    def allowMerge(self):
    ...
    def mergeNodesFromMenu(self):  
        self.node.sigMerge.emit(self)
    def saveNodesFromMenu(self):  
        self.node.sigSave.emit(self.node)
    def expandeNodeFromMenu(self):  
        self.node.sigExp.emit(self)    
