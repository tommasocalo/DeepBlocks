...
class Terminal(object):
    ...
    def connectTo(self, term, connectionItem=None):
        ...
        if term._refterm!=None:
            self._supercon[term] = connectionItem
            if term.last not in self.connections():

                conn = term.last.connectTo(self)
                conn.hide()
                term.last.recolor()
        else:
    ...
    def recolor(self, color=None, recurse=True):
        if color is None:
            if not self.isConnected():       ## disconnected terminals are black
                ...
                self.color = QtGui.QColor(200,200,0)
...
class TerminalGraphicsItem(GraphicsObject):
    ...
    def disconnect(self, target):
        if target.term._refterm!= None:
           target.term.disconnectFrom(self.term)
        else:
            ...
            if self.newConnection is not None:
                ...
                for i in items:
                    if isinstance(i, TerminalGraphicsItem):
                        ...
                        try:
                            if i.term._refterm!= None:
                                   i.term.connectTo(self.term,self.newConnection)
                            else:
    ...
    def nodeMoved(self):
        ...
        if len(self.term._supercon)>0:   
            for t, item in self.term._supercon.items():
                if item.isVisible():
                    item.updateLine()
        if len(self.term.connections())>0:
